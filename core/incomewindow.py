...
class income_window(tk.Tk):
    def __init__(self, time_data: list[str], time_raw: str) -> None:
        ...
        self.income_table_name_label = ttk.Label(
            self.income_table, text="Name")
        self.income_table_name_label.grid(row=0, column=0)

        self.income_table_value_label = ttk.Label(
            self.income_table, text="Value")
        self.income_table_value_label.grid(row=0, column=1)

        self.display_data()

        # add income frame
        add_income_frame = ttk.Frame(
            master=self, relief=tk.SOLID, borderwidth=2)
        add_income_frame.grid(row=10, column=1)

        # labels
        self.add_income_name_label = ttk.Label(
            add_income_frame, text="Income Name")
        self.add_income_name_label.grid(row=4, column=4)

        self.add_income_value_label = ttk.Label(
            add_income_frame, text="Income Value")
        self.add_income_value_label.grid(row=5, column=4)

        # text input
        self.add_income_name_input = Text(
            add_income_frame, height=1, width=20)
        self.add_income_name_input.grid(row=4, column=5)

        self.add_income_value_input = Text(
            add_income_frame, height=1, width=20)
        self.add_income_value_input.grid(row=5, column=5)
    ...
    def add_new_income_EV(self) -> None:
        name = self.add_income_name_input.get(1.0, tk.END+'-1c')
        value = self.add_income_value_input.get(1.0, tk.END+'-1c')

        dh.push_data(self.time_data[3], "incomes", name, value)
        self.display_data()

    def remove_income_EV(self) -> None:
        name = self.remove_income_name_input.get(1.0, tk.END+'-1c')
        value = self.remove_income_value_input.get(1.0, tk.END+'-1c')
