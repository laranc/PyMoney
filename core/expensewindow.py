...
class expense_window(tk.Tk):
    def __init__(self, time_data: list[str], time_raw: str) -> None:
        ...
        # labels
        self.add_expenses_name_label = ttk.Label(add_expenses_frame, text="Expense Name")
        self.add_expenses_name_label.grid(row=4, column=4)

        self.add_expenses_value_label = ttk.Label(add_expenses_frame, text="Expense Value")
        self.add_expenses_value_label.grid(row=5, column=4)

        # text input
        self.add_expenses_name_input = Text(add_expenses_frame, height=1, width=20)
        self.add_expenses_name_input.grid(row=4, column=5)

        self.add_expenses_value_input = Text(add_expenses_frame, height=1, width=20)
        self.add_expenses_value_input.grid(row=5, column=5)

        # submit data
        self.add_new_expense_button = ttk.Button(self, text="Add New Expense", command=self.add_new_expense_EV)
        ...
        self.remove_expenses_name_label = ttk.Label(add_expenses_frame, text="Expense Name")
        self.remove_expenses_name_label.grid(row=4, column=4)

        self.remove_expenses_value_label = ttk.Label(add_expenses_frame, text="Expense Value")
        self.remove_expenses_value_label.grid(row=5, column=4)

        # text input
        self.remove_expenses_name_input = Text(add_expenses_frame, height=1, width=20)
        self.remove_expenses_name_input.grid(row=4, column=5)

        self.remove_expenses_value_input = Text(add_expenses_frame, height=1, width=20)
        self.remove_expenses_value_input.grid(row=5, column=5)

        # submit data
        self.remove_new_expense_button = ttk.Button(self, text="Add New Expense", command=self.remove_expense_EV)
    ...
    def display_data(self) -> None:
        ...
        if len(json_data) == 0: # file empty
            ...
            for key in json_data[4]:
                ...
                value = ttk.Label(self.expenses_table, text=f"{json_data[4][key]}")
                value.grid(row=x, column=1)
                x += 1
    ...
    def add_new_expense_EV(self) -> None:
        ...
        dh.push_data(self.time_data[3], name, value)
        self.display_data()
